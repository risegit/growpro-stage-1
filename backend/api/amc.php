...
switch ($method) {
    ...
    case 'GET':
        if ($amcId) {
            ...
            $result = $conn->query("SELECT u.id AS customer_id, u.name, u.phone, cd.locality , a.id AS amc_id, a.status, a.amc_free_paid, a.visits_per_month, a.validity_from, a.validity_upto, COUNT(s.id) AS total_visits_done, (TIMESTAMPDIFF(MONTH, a.validity_from, a.validity_upto) * a.visits_per_month) AS total_allowed_visits, ((TIMESTAMPDIFF(MONTH, a.validity_from, a.validity_upto) * a.visits_per_month) - COUNT(s.id)) AS pending_visits, (SELECT COUNT(*) FROM site_visit sv WHERE sv.customer_id = u.id AND sv.created_date >= DATE_FORMAT(CURRENT_DATE, '%Y-%m-01') AND sv.created_date < DATE_FORMAT(CURRENT_DATE + INTERVAL 1 MONTH, '%Y-%m-01')) AS current_month_visits_done, (a.visits_per_month - (SELECT COUNT(*) FROM site_visit sv WHERE sv.customer_id = u.id AND sv.created_date >= DATE_FORMAT(CURRENT_DATE, '%Y-%m-01') AND sv.created_date < DATE_FORMAT(CURRENT_DATE + INTERVAL 1 MONTH, '%Y-%m-01'))) AS remaining_visits_current_month FROM users u INNER JOIN amc_details a ON u.id = a.customer_id LEFT JOIN site_visit s ON s.customer_id = u.id AND s.created_date BETWEEN a.validity_from AND a.validity_upto INNER JOIN customers_details cd ON a.customer_id=cd.user_id WHERE u.status = 'active' AND CURRENT_DATE <= a.validity_upto GROUP BY u.id, u.name, u.phone, a.id, a.amc_free_paid, a.visits_per_month, a.validity_from, a.validity_upto ORDER BY pending_visits DESC");
            $data = [];
