...
switch ($method) {
        ...
        case 'GET':
            if($siteVisitId){
                ...
                $sql1 = "SELECT sv.id onsite_id,u.id customer_id,u.name,u.phone,u.profile_pic,cd.locality,t.name tech_name,COALESCE(mdr.delivery_status, 'no') AS delivery_status, sv.created_date, mdr.updated_date FROM site_visit sv INNER JOIN users u ON sv.customer_id=u.id INNER JOIN users t ON sv.visited_by=t.user_code LEFT JOIN material_delivery_records mdr ON sv.id=mdr.visit_id INNER JOIN customers_details cd ON cd.user_id=u.id";
                $result = $conn->query($sql1);
                ...
                $offsiteSql1 = "SELECT omd.id offsite_id, omd.customer_id, omd.delivery_status, omd.created_date, omd.created_time, omd.created_by, u.name, cd.locality, u.phone,u.profile_pic,omd.updated_date FROM offsite_material_deliver omd INNER JOIN users u ON omd.customer_id=u.id INNER JOIN customers_details cd ON omd.customer_id=cd.user_id";
