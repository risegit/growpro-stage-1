...
switch ($method) {
    ...
    case 'GET':
        if ($userId) {
            ...
            $sql1 = "SELECT * FROM users INNER JOIN amc_details ON users.id=amc_details.customer_id WHERE users.status='active' and amc_details.validity_upto > CURRENT_DATE;";
            $result = $conn->query($sql1);
            $data = [];
            while ($row = $result->fetch_assoc()) {
                $data[] = $row;
